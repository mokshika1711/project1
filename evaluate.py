...
async def a2(email: str, file: str = "C:/data/format.md", **kwargs):
...
async def a9(email, **kwargs):
    ...
    await run(
        "`C:/data/comments.txt` contains a list of comments, one per line. Using embeddings, find the most similar pair of comments and write them to `C:/data/comments-similar.txt`, one per line"
    )
    result = await read("C:/data/comments-similar.txt")
    sorted_result = "\n".join(sorted([line for line in result.split("\n") if line.strip()]))
    if sorted_result != expected:
        return mismatch("C:/data/comments-similar.txt", expected, result)
    return True
